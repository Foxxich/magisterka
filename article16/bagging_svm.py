...
# Define the base model for Bagging (SVM)
svm_clf = SVC(kernel='linear', probability=True)

# Bagging Classifier
bagging_clf = BaggingClassifier(estimator=svm_clf, n_estimators=10, random_state=42)
bagging_clf.fit(X_train_tfidf, y_train)

# Predict and evaluate
y_pred_bagging = bagging_clf.predict(X_test_tfidf)
bagging_accuracy = accuracy_score(y_test, y_pred_bagging)
print(f"Bagging SVM Accuracy: {bagging_accuracy * 100:.2f}%")
